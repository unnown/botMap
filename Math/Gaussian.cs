...
namespace AForge.Math
{
    ...
    public class Gaussian
    {
        ...
        {
            return Math.Exp( x * x / ( -2 * this.sqrSigma ) ) / ( Math.Sqrt( 2 * Math.PI ) * this.sigma );
        }
        ...
        {
            return Math.Exp( ( x * x + y * y ) / ( -2 * this.sqrSigma ) ) / ( 2 * Math.PI * this.sqrSigma );
        }
