...
namespace AForge.Imaging
{
    ...
    public static class Drawing
    {
        ...
        public static unsafe void FillRectangle( UnmanagedImage image, Rectangle rectangle, Color color )
        {
            ...
            var startX  = Math.Max( 0, rectX1 );
            var stopX   = Math.Min( imageWidth - 1, rectX2 );
            var startY  = Math.Max( 0, rectY1 );
            var stopY   = Math.Min( imageHeight - 1, rectY2 );
        ...
        public static unsafe void Line( UnmanagedImage image, IntPoint point1, IntPoint point2, Color color )
        {
            ...
            if ( Math.Abs( dx ) >= Math.Abs( dy ) )
            {
                ...
                if ( image.PixelFormat == PixelFormat.Format8bppIndexed )
                {
                    ...
                    {
                        var px = startX + x;
                        var py = (int) ( (float) startY + ( slope * (float) x ) );
            ...
            else
            {
                ...
                if ( image.PixelFormat == PixelFormat.Format8bppIndexed )
                {
                    ...
                    {
                        var px = (int) ( (float) startX + ( slope * (float) y ) );
                        var py = startY + y;
