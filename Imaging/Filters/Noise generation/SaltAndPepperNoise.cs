...
namespace AForge.Imaging.Filters
{
    ...
    public class SaltAndPepperNoise : BaseInPlacePartialFilter
    {
        ...
        protected override unsafe void ProcessFilter( UnmanagedImage image, Rectangle rect )
        {
            ...
            var noisyPixels = (int) ( ( width * height * this.noiseAmount ) / 100 );
            ...
            if ( image.PixelFormat == PixelFormat.Format8bppIndexed )
            {
                ...
                {
                    var x = startX + this.rand.Next( width );
                    var y = startY + this.rand.Next( height );
            ...
            else
            {
                ...
                for ( var i = 0; i < noisyPixels; i++ )
                {
                    var x = startX + this.rand.Next( width );
                    var y = startY + this.rand.Next( height );
                    var colorPlane = this.rand.Next( 3 );
