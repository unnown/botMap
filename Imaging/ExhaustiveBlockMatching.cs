...
namespace AForge.Imaging
{
    ...
    public class ExhaustiveBlockMatching : IBlockMatching
    {
        ...
        public List<BlockMatch> ProcessImage( UnmanagedImage sourceImage, List<IntPoint> coordinates, UnmanagedImage searchImage )
        {
            ...
            var blockRadius = this.blockSize / 2;
            var searchWindowSize = 2 * this.searchRadius;
            var blockLineSize = this.blockSize * pixelSize;
            var blockOffset = stride - ( this.blockSize * pixelSize );

            // maximum possible difference of blocks
            var maxDiff = this.blockSize * this.blockSize * pixelSize * 255;
            ...
            unsafe
            {
                ...
                for ( var iPoint = 0; iPoint < pointsCount; iPoint++ )
                {
                    ...
                    var searchStartX = refPointX - blockRadius - this.searchRadius;
                    var searchStartY = refPointY - blockRadius - this.searchRadius;
                    ...
                    for ( var searchWindowRow = 0; searchWindowRow < searchWindowSize; searchWindowRow++ )
                    {
                        ...
                        for ( var searchWindowCol = 0; searchWindowCol < searchWindowSize; searchWindowCol++ )
                        {
                            ...
                            if ( ( blockSearchX < 0 ) || ( blockSearchY + this.blockSize >= width ) )
                            {
                                // skip column
                                continue;
                            }
                            ...
                            var error = 0;
                            for ( var blockRow = 0; blockRow < this.blockSize; blockRow++ )
                            {
