...
namespace AForge.Imaging.ColorReduction
{
    ...
    public abstract class ErrorDiffusionColorDithering
    {
        ...
        public unsafe Bitmap Apply( UnmanagedImage sourceImage )
        {
            ...
            var offset = this.stride - this.width * this.pixelSize;

            // create destination image
            var destImage = new Bitmap(this.width , this.height , ( this.colorTable.Length > 16 ) ?
            ...
            var destData = destImage.LockBits( new Rectangle( 0, 0, this.width , this.height ),
            ...
            var is8bpp = ( this.colorTable.Length > 16 );

            // for each line
            for ( this.y = 0; this.y < this.height; this.y++ )
            {
                var dst = dstBase + this.y * destData.Stride;
                ...
                for ( this.x = 0; this.x < this.width; this.x++, ptr += this.pixelSize )
                {
                    ...
                    var closestColor = this.GetClosestColor( r, g, b, out colorIndex );
        ...
        private Color GetClosestColor( int red, int green, int blue, out byte colorIndex )
        {
            ...
            else
            {
                ...
                for ( int i = 0, n = this.colorTable.Length; i < n; i++ )
                {
                    var dr = red - this.colorTable[i].R;
                    var dg = green - this.colorTable[i].G;
                    var db = blue - this.colorTable[i].B;
