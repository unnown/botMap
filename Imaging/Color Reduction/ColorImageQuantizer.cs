...
namespace AForge.Imaging.ColorReduction
{
    ...
    public class ColorImageQuantizer
    {
        ...
        public Bitmap ReduceColors( Bitmap image, int paletteSize )
        {
            ...
            try
            {
                var result = this.ReduceColors( new UnmanagedImage( data ), paletteSize );
        ...
        public Bitmap ReduceColors( Bitmap image, Color[] palette )
        {
            ...
            try
            {
                var result = this.ReduceColors( new UnmanagedImage( data ), palette );
                if ( ( image.HorizontalResolution > 0 ) && ( image.VerticalResolution > 0 ) )
                {
                    result.SetResolution( image.HorizontalResolution, image.VerticalResolution );
                }
            ...
            unsafe
            {
                ...
                for ( var y = 0; y < height; y++ )
                {
                    ...
                    for ( var x = 0; x < width; x++, ptr += pixelSize )
                    {
                        ...
                        var colorIndex = (byte)this.GetClosestColor( ptr[RGB.R], ptr[RGB.G], ptr[RGB.B] );
        ...
        private int GetClosestColor( int red, int green, int blue )
        {
            ...
            {
                var dr = red - this.paletteToUse[i].R;
                var dg = green - this.paletteToUse[i].G;
                var db = blue - this.paletteToUse[i].B;
