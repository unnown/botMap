...
namespace AForge.Imaging
{
    ...
    public class UnmanagedImage : IDisposable
    {
        ...
        public UnmanagedImage Clone( )
        {
            ...
            var newImageData = System.Runtime.InteropServices.Marshal.AllocHGlobal(this.stride * this.height );
            System.GC.AddMemoryPressure(this.stride * this.height );

            var newImage = new UnmanagedImage( newImageData, this.width , this.height , this.stride , this.pixelFormat );
            newImage.mustBeDisposed = true;

            AForge.SystemTools.CopyUnmanagedMemory( newImageData, this.imageData , this.stride * this.height );
        ...
        public void Copy( UnmanagedImage destImage )
        {
            ...
            {
                // copy entire image
                AForge.SystemTools.CopyUnmanagedMemory( destImage.imageData, this.imageData , this.stride * this.height );
            }
            else
            {
                unsafe
                {
                    ...
                    var copyLength = ( this.stride < dstStride ) ? this.stride : dstStride;

                    var src = (byte*)this.imageData.ToPointer( );
        ...
        public Bitmap ToManagedImage( bool makeCopy )
        {
            ...
            try
            {
                if ( !makeCopy )
                {
                    dstImage = new Bitmap(this.width , this.height , this.stride , this.pixelFormat , this.imageData );
                ...
                else
                {
                    ...
                    dstImage = ( this.pixelFormat == PixelFormat.Format8bppIndexed ) ?
                        AForge.Imaging.Image.CreateGrayscaleImage(this.width , this.height ) :
                    ...
                    var lineSize  = Math.Min(this.stride , dstStride );
                    ...
                    unsafe
                    {
                        ...
                        {
                            AForge.SystemTools.CopyUnmanagedMemory( dst, src, this.stride * this.height );
                        }
                    }
        ...
        {
            var pixelSize = Bitmap.GetPixelFormatSize(this.pixelFormat ) / 8;
            ...
            var pixelValues = new byte[points.Count * ( ( this.pixelFormat == PixelFormat.Format8bppIndexed ) ? 1 : 3 )];

            unsafe
            {
                var basePtr = (byte*)this.imageData.ToPointer( );
                byte* ptr;
        ...
        public List<IntPoint> CollectActivePixels( Rectangle rect )
        {
            ...
            unsafe
            {
                ...
                if ( ( this.pixelFormat == PixelFormat.Format16bppGrayScale ) || ( pixelSize > 4 ) )
                {
                    ...
                    {
                        var ptr = (ushort*) ( basePtr + y * this.stride + startX * pixelSize );
                ...
                else
                {
                    for ( var y = startY; y < stopY; y++ )
                    {
                        var ptr = basePtr + y * this.stride + startX * pixelSize;
                    ...
                    case PixelFormat.Format8bppIndexed:
                        {
                            ...
                            foreach ( var point in coordinates )
                            {
                                ...
                                {
                                    var ptr = basePtr + point.Y * this.stride + point.X;
                                    *ptr = grayValue;
                                }
                            }
                        }
                    ...
                    case PixelFormat.Format32bppRgb:
                        {
                            ...
                            foreach ( var point in coordinates )
                            {
                                if ( ( point.X >= 0 ) && ( point.Y >= 0 ) && ( point.X < this.width ) && ( point.Y < this.height ) )
                                {
                                    var ptr = basePtr + point.Y * this.stride + point.X * pixelSize;
                                    ptr[RGB.R] = red;
                                    ptr[RGB.G] = green;
                                    ptr[RGB.B] = blue;
                                }
                    ...
                    case PixelFormat.Format16bppGrayScale:
                        {
                            ...
                            foreach ( var point in coordinates )
                            {
                                if ( ( point.X >= 0 ) && ( point.Y >= 0 ) && ( point.X < this.width ) && ( point.Y < this.height ) )
                                {
                                    var ptr = (ushort*) ( basePtr + point.Y * this.stride ) + point.X;
                                    *ptr = grayValue;
                                }
                            }
                        }
                    ...
                    case PixelFormat.Format48bppRgb:
                        {
                            ...
                            foreach ( var point in coordinates )
                            {
                                if ( ( point.X >= 0 ) && ( point.Y >= 0 ) && ( point.X < this.width ) && ( point.Y < this.height ) )
                                {
                                    var ptr = (ushort*) ( basePtr + point.Y * this.stride + point.X * pixelSize );
                                    ptr[RGB.R] = red16;
                                    ptr[RGB.G] = green16;
                                    ptr[RGB.B] = blue16;
                                }
        ...
        private void SetPixel( int x, int y, byte r, byte g, byte b, byte a )
        {
            if ( ( x >= 0 ) && ( y >= 0 ) && ( x < this.width ) && ( y < this.height ) )
            {
                ...
                {
                    var pixelSize = Bitmap.GetPixelFormatSize(this.pixelFormat ) / 8;
                    var ptr = (byte*)this.imageData.ToPointer( ) + y * this.stride + x * pixelSize;
                    var ptr2 = (ushort*) ptr;
        ...
        public ushort[] Collect16bppPixelValues( List<IntPoint> points )
        {
            ...
            var pixelValues = new ushort[points.Count * ( ( this.pixelFormat == PixelFormat.Format16bppGrayScale ) ? 1 : 3 )];
