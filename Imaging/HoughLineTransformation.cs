                ...
                for ( var y = startY; y < stopY; y++ )
                {
                    ...
                    for ( var x = startX; x < stopX; x++, src++ )
                    {
                        if ( *src != 0 )
                        {
                            ...
                            {
                                var radius = (int) Math.Round(this.cosMap[theta] * x - this.sinMap[theta] * y ) + halfHoughWidth;
            ...
            for ( var theta = 0; theta < maxTheta; theta++ )
            {
                ...
                for ( var radius = 0; radius < maxRadius; radius++ )
                {
                    ...
                    if ( !foundGreater )
                    {
                        ...
                        this.lines.Add( new HoughLine( (double) theta / this.stepsPerDegree , (short) ( radius - halfHoughWidth ), intensity, (double) intensity / this.maxMapIntensity ) );
                    }
                }
            }
